...
Coarse_signatures <- list(
    Lymphocyte = c("PTPRC","CORO1A","RAC2"),
...
Fine_signatures<- list(
    ...
    Myeloid = c("MARCO","LYZ","MSR1","CD14","FCGR3A"),
...
VizGenEx(Coarse_signatures$Lymphocyte)  
```
...
VizGenEx(Fine_signatures$Myeloid) 
...
plotReducedDim(sce, dimred="MNN_UMAP_reduced", colour_by="ManualAnno_L1",point_alpha=0.7,point_size=0.1)+
  scale_colour_manual( values=meta_colors$celltype_level1[seq.int(length(unique(sce$ManualAnno_L1)))],name = "Annotation" )+ 
  labs( x='UMAP 1', y='UMAP 2' )+
...
```{r PlotL0}
#Plot manual Annotation
plotReducedDim(sce, dimred="MNN_UMAP_reduced", colour_by="ManualAnno_L0",point_alpha=0.7,point_size=0.1)+
  scale_colour_manual( values=meta_colors$celltype_level0[seq.int(length(unique(sce$ManualAnno_L0)))],name = "Annotation" )+ 
  labs( x='UMAP 1', y='UMAP 2' )+
  guides(colour = guide_legend(override.aes = list(size=2)))
```
...
for (labeling in c('ManualAnno_L0','ManualAnno_L1')){
  ...
  # Get the top 100 markers for each Anno (save in excel file)
  # filter for nominally significant (p<0.05) and over-expressed (auc>0.5)
