...
```{r }
#construct KNN graph
sce.milo <- buildGraph(sce.milo, k = 40, d = elbow, reduced.dim = "MNN")
...
sce.milo <- makeNhoods(sce.milo, prop = 0.05, k = 40, d = elbow, refined = TRUE, reduced_dims = "MNN",refinement_scheme="graph")
...
sce.milo <- calcNhoodDistance(sce.milo, d=elbow, reduced.dim = "MNN")

```

```{r }
# Testing
res <- testNhoods(sce.milo, design = ~ 0 + Joint.Location, design.df = design, model.contrasts = "Joint.LocationKnee-Joint.LocationWrist", reduced.dim = "MNN", fdr.weighting="graph-overlap") # fdr.weighting="graph-overlap"
...
umap_pl <- plotReducedDim(sce.milo, dimred = "MNN_UMAP", colour_by="label", 
                          text_size = 3, point_size=0.5) +
  guides(fill="none")

## Plot neighbourhood graph
nh_graph_pl <- plotNhoodGraphDA(sce.milo, res, layout="MNN_UMAP",alpha=0.1) 
