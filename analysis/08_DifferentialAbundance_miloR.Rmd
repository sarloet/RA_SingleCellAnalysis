...
plotCellTypeProps(sce.prop)

propeller.anova(prop.list=props, design=design, coef = c(1,2,3), robust=TRUE, trend=FALSE, sort=TRUE)

props <- getTransformedProps(sce.prop$cluster, sce.prop$Sample, transform="asin")
...
sce.milo <- buildGraph(sce.milo, k = 40, d = dim, reduced.dim = "MNN_reduced")
...
```{r }
# Testing
res <- testNhoods(sce.milo, design = ~ 0 + Joint.Location, design.df = design, model.contrasts = "Joint.LocationKnee-Joint.LocationMCP", reduced.dim = "MNN_reduced", fdr.weighting="graph-overlap") # fdr.weighting="graph-overlap"
...
umap_pl <- plotReducedDim(sce.milo, dimred = "MNN_UMAP_reduced", 
