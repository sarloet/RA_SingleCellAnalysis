...
```{r UMAPCluster}
#cluster
plotReducedDim(sce, paste0("MNN_UMAP_SC_reduced"),colour_by = "Annotation_L2", text_by="Annotation_L2") +labs(title="SubCluster ",subtitle="Colored by clustering")
```



## Profile Subcluster Celltypes

### Get DEG using Wilcoxon Rank sum test

```{r ClusterInfo}
  markers_wilcox <- wilcoxauc(sce, group_by="ManualAnno_L2", assay = 'logcounts')
  #topDEgenes <- top_markers(markers_wilcox, n = 100, auc_min = 0.5, pval_max = 0.05)
```

### Run GSEA to profile functions

```{r ClusterInfo}
for(label in unique(sce$ManualAnno_L2)){
  
  #Create ranked list for Gsea
  ranked_list <- markers_wilcox %>%   # Replace with actual subcluster name
               filter(group == label) %>%
               arrange(desc(logFC)) %>%
  ...
  gene_list <- ranked_list$logFC
  names(gene_list) <- ranked_list$feature
  ...
  gsea_result<-clusterProfiler::simplify(gsea_result)
  
  print(gsea_result@result[gsea_result@result$p.adjust<0.5,, drop=F])
}
