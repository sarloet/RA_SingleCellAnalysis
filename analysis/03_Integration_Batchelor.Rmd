...
#Run UMAP
sce <- runUMAP(sce ,name = "MNN_UMAP3", dimred = "MNN", min_dist = 0.3)
sce <- runUMAP(sce ,name = "MNN_UMAP_reduced3", dimred = "MNN_reduced", min_dist = 0.3)
```
...
```{r PlotUMAPAB3}
#Plot UMAP after data integration
plotReducedDim(sce, dimred="MNN_UMAP", colour_by="Joint.Location") + ggtitle("UMAP after data integration with fastMNN")
```
...
plotReducedDim(sce, dimred="MNN_UMAP", colour_by="Sample") + ggtitle("UMAP after data integration with fastMNN")+ facet_wrap(~sce.BatchR$Sample)+ theme(strip.background=element_rect(fill="white"))
```
...
plotReducedDim(sce, dimred="MNN_UMAP", colour_by="Joint.Location") + ggtitle("UMAP after data integration with fastMNN")+ facet_wrap(~sce.BatchR$Joint.Location)+ theme(strip.background=element_rect(fill="white"))
```
