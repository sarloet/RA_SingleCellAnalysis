...
#Perform fastMNN
bpstart(bpp)
sce.Batchelor <- fastMNN(sce.Batchelor, batch=sce.Batchelor$Sample, prop.k=0.05,  subset.row = rownames(sce.Batchelor)[rowData(sce.Batchelor)[["is_hvg"]]], correct.all=TRUE,BPPARAM = bpp)#merge.order = merge_order,
bpstop(bpp)
...
elbow <- ceiling(as.numeric(maxLikGlobalDimEst(as.matrix(reducedDim(sce.BatchR, "MNN")), k=10)))

#PCA select nr of PC
reducedDim(sce.BatchR,"MNN_reduced") <- reducedDim(sce.BatchR,"MNN")[,seq_len(elbow)]
reducedDimNames(sce.BatchR)

ncol(reducedDim(sce.BatchR,"MNN"))
ncol(reducedDim(sce.BatchR,"MNN_reduced"))
...
plotReducedDim(sce.BatchR, dimred="MNN_reduced", colour_by="Sample") + ggtitle("PCA after data integration with fastMNN")
```
...
```{r PlotPCAAB2}
#Plot PCA after data integration
plotReducedDim(sce.BatchR, dimred="MNN_reduced", colour_by="Joint.Location") + ggtitle("PCA after data integration with fastMNN")
```
...
plotReducedDim(sce.BatchR, dimred="MNN_reduced", colour_by="Sample") + ggtitle("PCA after data integration with fastMNN")+ facet_wrap(~sce.BatchR$Sample)+ theme(strip.background=element_rect(fill="white"))
```
...
plotReducedDim(sce.BatchR, dimred="MNN_reduced", colour_by="Joint.Location") + ggtitle("PCA after data integration with fastMNN")+ facet_wrap(~sce.BatchR$Joint.Location)+ theme(strip.background=element_rect(fill="white"))
```
...
```{r PlotUMAPAB1}
#Run UMAP
sce.BatchR <- runUMAP(sce.BatchR ,name = "MNN_UMAP1", dimred = "MNN", min_dist = 0.1)
sce.BatchR <- runUMAP(sce.BatchR ,name = "MNN_UMAP_reduced1", dimred = "MNN_reduced", min_dist = 0.1)


#Run UMAP
sce.BatchR <- runUMAP(sce.BatchR ,name = "MNN_UMAP", dimred = "MNN", min_dist = 0.2)
sce.BatchR <- runUMAP(sce.BatchR ,name = "MNN_UMAP_reduced", dimred = "MNN_reduced", min_dist = 0.2)

#Run UMAP
sce.BatchR <- runUMAP(sce.BatchR ,name = "MNN_UMAP3", dimred = "MNN", min_dist = 0.3)
sce.BatchR <- runUMAP(sce.BatchR ,name = "MNN_UMAP_reduced3", dimred = "MNN_reduced", min_dist = 0.3)
```
...
plotReducedDim(sce.BatchR, dimred="MNN_UMAP_reduced", colour_by="Sample") + ggtitle("UMAP after data integration with fastMNN")
```
...
```{r SaveData}
saveRDS(sce.BatchR, file =paste0(path,'/output/03_sce_Integration_Batchelor.rds'))
```
