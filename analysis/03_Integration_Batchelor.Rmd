...
pca_pct_variance <- data.frame(variance = attr(reducedDim(sce, "MNN"), "percentVar"))
pca_pct_variance$PC <- 1:nrow(pca_pct_variance)

#PCA dim estimate using intrinsicDimension::maxLikGlobalDimEst
intrinsicDimension::maxLikGlobalDimEst(as.matrix(reducedDim(sce, "MNN")), k=10)
...
```{r SelectPC}
#PCA select nr of PC
reducedDim(sce,"MNN_reduced") <- reducedDim(sce,"MNN")[,seq_len(12)]
reducedDimNames(sce)
...
```{r PlotUMAPAB1}
#Run UMAP
sce.BatchR <- runUMAP(sce.BatchR ,name = "MNN_UMAP", dimred = "MNN_reduced")
