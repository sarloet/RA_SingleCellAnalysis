...
#sce <- sce[,sce$Annotation_L1 == "Fibroblast"]
#sce <- sce[,sce$Annotation_L2 %in% c("FOLR2+ MP","TREM2+ MP")]
#sce <- sce[,sce$Annotation_L1 == "Endothelial cell"]
sce <- sce[,sce$Annotation_L1 == "Smooth muscle cell"]
```
...
for (name in c("geneList_KneeVsMCP","geneList_KneeVsWrist","geneList_MCPVsWrist")){
  
  tbl<-eval(parse(text =name))
  tbl<- tbl[tbl$FDR<0.05 & abs(tbl$log2FoldChange) > 0.1,, drop=F]
  glist<-c(glist, tbl$primerid )
}
glist<-unique(glist)
assign(x = paste0("MAST"),value =glist)

```

```{r DEwMAST}
listInput <- list(edgeR = edgeR, DESeq2 = DESeq2, limmavoom = `limma-voom`, edgeRglm = edgeR_glm,MAST=MAST)
upset(fromList(listInput), order.by = "freq")
```

```{r DEwMAST}
#Give Genes intersecting
Reduce(intersect,list(edgeR = edgeR, DESeq2 = DESeq2, limmavoom = `limma-voom`, edgeRglm = edgeR_glm,MAST=MAST))
Reduce(intersect,list(edgeR = edgeR, DESeq2 = DESeq2, limmavoom = `limma-voom`,MAST=MAST))
