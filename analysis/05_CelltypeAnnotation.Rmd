...
dittoDotPlot(sce.marker, vars = top_genes_wilcox , group.by = "label",min.color="white", max.color="red",adjustment="z-score",size=5)
  ...
  for(i in markr){
    ...
    print(gridExtra::grid.arrange(
      arrangeGrob(plotReducedDim(sce, dimred = "MNN_UMAP", colour_by = i, text_by = "cluster"), 
                  plotReducedDim(sce, dimred = "MNN_UMAP", colour_by = i, order_by=i ) +scale_colour_gradient(name = i,low = "lightgrey", high = "red"), ncol = 2), 
...
for(i in markr){
  ...
  cat(paste0("#### ", i, "\n"))
  print(
  plotReducedDim(sce, dimred = "MNN_UMAP", colour_by = i, text_by = "cluster")
  )
  cat('\n\n')
...
```{r SaveData}
saveRDS(sce, file =paste0(path,'/output/05_CelltypeAnnotation.rds'))
```
