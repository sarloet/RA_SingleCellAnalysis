...
```{r PlotClusterPCA}
plotReducedDim(sce, dimred="MNN_reduced", colour_by="cluster",text_by="cluster") + ggtitle("UMAP with cluster label")
```

### UMAP

```{r PlotClusterUMAP}
plotReducedDim(sce, dimred="MNN_UMAP_reduced", colour_by="cluster",text_by="cluster") + ggtitle("UMAP with cluster label")
```

### QC UMAPs

```{r PlotClusterQCUMAP,fig.dim = c(8, 6)}
p1 <- plotReducedDim(sce, dimred="MNN_UMAP_reduced", colour_by="subsets_Mito_percent") + ggtitle("Mitochondrial Genes")
p2 <- plotReducedDim(sce, dimred="MNN_UMAP_reduced", colour_by="detected") + ggtitle("Number of detected Genes ")
p3 <- plotReducedDim(sce, dimred="MNN_UMAP_reduced", colour_by="sum") + ggtitle("Sum of UMI Counts in Cell")
...
dittoHeatmap(sce.plot, vars = unique(top3DEgenes), annot.by = c("cluster"),order.by="cluster", scaled.to.max = TRUE,cluster_rows=FALSE,cluster_cols=FALSE,heatmap.colors = colorRampPalette(c("dark blue", "white", "dark red"))(100),breaks = seq(-3, 3, length.out = 101 ) #assay = "scaled_asinh"
  ...
  for(i in markr){
    ...
    gridExtra::grid.arrange(
      arrangeGrob(plotReducedDim(sce, dimred = "MNN_UMAP_reduced", colour_by = i, text_by = "cluster"), 
                  plotReducedDim(sce, dimred = "MNN_UMAP_reduced", colour_by = i, order_by=i ) 
...
sce$ManualAnno_L0[sce$cluster %in% c(5,12,13,16,17)] <- "Lymphocyte"
sce$ManualAnno_L0[sce$cluster %in% c(4,7,8,9,15,19)] <- "Myeloid"
sce$ManualAnno_L0[sce$cluster %in% c(2,3,6,10,11)] <- "Stromal"
sce$ManualAnno_L0[sce$cluster %in% c(1,14,18)] <- "Endothelial"
...
sce$ManualAnno_L1[sce$cluster %in% c(2,3,6,10,11)] <- "Fibroblast"
#sce$ManualAnno_L1[sce$cluster %in% c(15)] <- "Smooth muscle cell"
sce$ManualAnno_L1[sce$cluster %in% c(1,14,18)] <- "Endothelial cell"
sce$ManualAnno_L1[sce$cluster %in% c(12,13,16)] <- "T cell"
sce$ManualAnno_L1[sce$cluster %in% c(17)] <- "B cell"
sce$ManualAnno_L1[sce$cluster %in% c(19)] <- "Neutrophil"
#sce$ManualAnno_L1[sce$cluster %in% c(16)] <- "Mast cell"
sce$ManualAnno_L1[sce$cluster %in% c(4,7,8,9,15)] <- "Myeloid"
...
ggplot(Totalcelltype, aes( y=freq, fill=Joint.Location)) + 
  geom_boxplot()+
  facet_wrap(~celltype, scales = "free_y",ncol = 5, nrow = 2) +
  labs(title="Relative cell type abundance", x="", y="")+ 
  theme_classic()



# Fraction of cells [%] from total cells in that joint location

Totalcelltype <- as.data.frame(colData(sce)) %>% 
    group_by(Joint.Location) %>%
    summarise(totalcells = n(),.groups = 'drop') %>% 
    left_join(ClusterInfo%>% group_by(Joint.Location,celltype), Totalcelltype,by ="Joint.Location") %>%
    mutate(freq = cells / totalcells * 100)


ggplot(Totalcelltype, aes(x= celltype , y=freq, fill=Joint.Location)) + 
    geom_boxplot()+ 
    theme_classic()
...
Totalcelltype <- as.data.frame(colData(sce)) %>% 
    group_by(celltype) %>%
    summarise(totalcells = n(),.groups = 'drop') %>% 
    left_join(ClusterInfo, Totalcelltype,by ="celltype") %>%
    mutate(freq = cells / totalcells * 100)
