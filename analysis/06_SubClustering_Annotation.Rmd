...
subtitle: "06_SubClustering_Annotation"
author: "SarahL."
...
VizGenEx <- function(sce.sc,markr,Ctype){
  for(i in markr){
    
    cat(paste0("#### ", i, "\n"))
    gridExtra::grid.arrange(
      arrangeGrob(plotReducedDim(sce.sc[,sce.sc$celltype == Ctype], dimred = "MNN_UMAP_SC_reduced", colour_by = i, text_by = "subcluster"), 
                  plotReducedDim(sce.sc[,sce.sc$celltype == Ctype], dimred = "MNN_UMAP_SC_reduced", colour_by = i, order_by=i ) 
...
Wilcoxauc <- function(sce,label,celltype,n = 50,auc.min = 0.5, pval.max = 0.05, Create.Table=TRUE){
  ...
  if(Create.Table==TRUE){
    for (i in 1:max(as.integer(levels(sce$paste0(label))))){
...
```{r LoadDataset}
## RA DATASET
sce.sc <- readRDS(file = paste0(path,'/output/06_sce_SC_Fibroblast.rds'))
...
```{r FibroblastsL2, results='asis'}
#Lining Fibroblst
VizGenEx(sce.sc,c("PRG4","CRTAC1", "CLU","CLIC5"),"Fibroblast")  
...
```{r FibroblastsL2, results='asis'}
#SMC
VizGenEx(sce.sc,c("ACTA2","MYL9", "DES", "MYH11"),"Fibroblast")  
...
```{r FibroblastsL2, results='asis'}
#Lining Fibroblst
VizGenEx(sce.sc,c("CHI3L2","SAA1", "FN1"),"Fibroblast")  
...
```{r FibroblastsL2, results='asis'}
#Lining Fibroblst
VizGenEx(sce.sc,c("CXCL12","CCL2","SFRP1"),"Fibroblast")  
...
```{r FibroblastsL2, results='asis'}
#Lining Fibroblst
VizGenEx(sce.sc,c("POSTN","COL1A1", "FNDC1"),"Fibroblast")  
...
```{r FibroblastsL2, results='asis'}
#Lining Fibroblst
VizGenEx(sce.sc,c("CXCL14","MFAP5"),"Fibroblast")  
...
```{r SaveData}
saveRDS(sce.sc, file =paste0(path,'/output/06_sce_SC_Fibroblast_Anno.rds'))
```
...
```{r SaveData}
saveRDS(sce, file =paste0(path,'/output/06_sce_SC_Fibroblast_Anno.rds'))
```
