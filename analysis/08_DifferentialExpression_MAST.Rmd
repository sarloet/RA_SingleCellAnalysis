...
#Output path
output_dir <- here("output","MAST_output")
...
```{r UMAPAnnotation}
#cell type
plotReducedDim(sce, dimred="MNN_merge_reduced_UMAP", colour_by="Annotation_L1",point_alpha=0.7,point_size=0.1)+
  labs( x='UMAP 1', y='UMAP 2',title="UMAP colored by Celltype",subtitle = "UMAP of all integrated datasets"  )+
...
```{r UMAPCluster}
#cell type
plotReducedDim(sce, dimred="MNN_merge_reduced_UMAP", colour_by="cluster",point_alpha=0.7,point_size=0.1)+
  labs( x='UMAP 1', y='UMAP 2',title="UMAP colored by clusters",subtitle = "UMAP of all integrated datasets" )+
  guides(colour = guide_legend(override.aes = list(size=2)))
```
...
```{r SaveData}
sce.subset <- sce[,sce$Annotation_L1 == "Fibroblast"]
dim(sce.subset)
```
...
## Save the datasets FB

```{r SaveData}
write.csv(res_KneeVsMCP, paste0(output_dir,'/MAST_KneeVsMCP_FB.csv'), row.names=FALSE)
write.csv(res_KneeVsWrist, paste0(output_dir,'/MAST_KneeVsWrist_FB.csv'), row.names=FALSE)
write.csv(res_MCPVsWrist, paste0(output_dir,'/MAST_MCPVsWrist_FB.csv'), row.names=FALSE)
```
...
```{r SaveData}
#Get only Tissue Residnent Macrophages
sce.subset <- sce[,sce$Annotation_L1 == "Macrophage"]

sce.subset <- sce.subset[,sce.subset$Annotation_L2 != "CD48+ MP"]
sce.subset <- sce.subset[,sce.subset$Annotation_L2 != "S100A12+ MP"]
dim(sce.subset)
```
...
for (comp in c("KneeVsMCP","KneeVsWrist","MCPVsWrist")){
  ...
  volcano_plot<-EnhancedVolcano(res_plot, lab = row.names(res_plot), x = 'log2FoldChange', y = 'PValue',
                  pCutoff = 0.05, FCcutoff = 0.5,
                  title = paste("Differential expression", strspl[[1]][1], "vs", strspl[[1]][2], sep=' '),
                  subtitle = paste0("Macrophage"," cluster (MAST)"),
                  pointSize = 1.0, colAlpha = 1,
                  legendPosition = 'right',
                  gridlines.major = FALSE,
                  gridlines.minor = FALSE)+
...
## Save the datasets MP

```{r SaveData}
write.csv(res_KneeVsMCP, paste0(output_dir,'/MAST_KneeVsMCP_MP.csv'), row.names=FALSE)
write.csv(res_KneeVsWrist, paste0(output_dir,'/MAST_KneeVsWrist_MP.csv'), row.names=FALSE)
write.csv(res_MCPVsWrist, paste0(output_dir,'/MAST_MCPVsWrist_MP.csv'), row.names=FALSE)
```


--------------------------------------------------------------------------------------------------------------------------------------------------------

## Run MAST Endothelial 

```{r SaveData}
#Get only Tissue Residnent Macrophages
sce.subset <- sce[,sce$Annotation_L1 == "Endothelial cell"]
...
## Save the datasets EC

```{r SaveData}
write.csv(res_KneeVsMCP, paste0(output_dir,'/MAST_KneeVsMCP_EC.csv'), row.names=FALSE)
write.csv(res_KneeVsWrist, paste0(output_dir,'/MAST_KneeVsWrist_EC.csv'), row.names=FALSE)
write.csv(res_MCPVsWrist, paste0(output_dir,'/MAST_MCPVsWrist_EC.csv'), row.names=FALSE)
```
