...
sce <- sce[rowSums(counts(sce)> 0) > 0, colSums(counts(sce)> 0) > 150] #200
...
#Load data as sce object
sce.CR<-read10xCounts(samples = filepaths, sample.names = foldernames,col.names=T,type="HDF5")

#Edit Filenames 
colData(sce.CR)$Sample = sub("\\_w_introns.*", "", sub("cellbender_output_", "", as.character(colData(sce.CR)$Sample)))


#Add metadata to sce
colData(sce.CR) <- dplyr::left_join(as.data.frame(colData(sce.CR)),
                                   metadata_df, 
                                   by= c("Sample" = "Sample"),
                                   suffix=c(".x",".y")) %>% 
      dplyr::select(-one_of("Comments")) %>% #select all except
      DataFrame(row.names=colnames(sce.CR))

#Set Sample names
names(colData(sce.CR))[which(names(colData(sce.CR))=="Sample")]="Orig.Identifier"
names(colData(sce.CR))[which(names(colData(sce.CR))=="ID")]="Sample"

#make col names unique
colnames(sce.CR) <- paste0(sce.CR$Sample, ".", sce.CR$Barcode)
...
```{r InitialFiltering}
#Initial filtering before droplet removal
dim_before_filtering <- dim(sce.CR)

#Get only the detected Genes
#Remove Cells with very low counts of less than 150 and genes not expressed 
sce.CR <- sce.CR[rowSums(counts(sce.CR)> 0) > 0, colSums(counts(sce.CR)> 0) > 200] #200


dim_after_filtering <- dim(sce.CR)
...
#Detection
sce.CR <- scDblFinder::scDblFinder(sce.CR, samples=sce.CR$Sample, BPPARAM = bpp) 

table(sce.CR$scDblFinder.class)
as.data.frame.matrix(table(sce.CR$Sample,sce.CR$scDblFinder.class))
