...
```{r LoadDataset}
## RA DATASET
sce.sc <- readRDS(file = paste0(path,'/output/07_sce_SC_Myeloid.rds'))
...
Myeloid_signatures <- list(
    TREM2 = c("MERTK","TREM2"),#Tissue resident
    FOLR2 = c("MERTK","FOLR2","LYVE1","ID2","ICAM1"),#Tissue resident
    CD48 = c("CD48","SPP1","ISG15","CLEC10A"),#Tissue infiltrating
    S100A12 = c("S100A12","CD48","MRC1","FCGR3A")#Monocyte (Downregulated MRC1, FCGR3A)
)
```
...
```{r UCellCoarse}
sce.ucell <- ScoreSignatures_UCell(sce.sc, features=Myeloid_signatures, assay = 'counts', name=NULL, BPPARAM=bpp)
altExp(sce.ucell, 'UCell')
...
#Macrophage
VizGenEx(sce.sc,Myeloid_signatures$TREM2)  
```


```{r TcellL2, results='asis'}
VizGenEx(sce.sc,Myeloid_signatures$FOLR2)  
```


```{r TcellL2, results='asis'}
VizGenEx(sce.sc,Myeloid_signatures$CD48)  
```

```{r TcellL2, results='asis'}
#monocyte
VizGenEx(sce.sc,Myeloid_signatures$S100A12)  
...
#sce.sc$ManualAnno_L2[sce.sc$subcluster %in% c(6)] <- "SMC"
sce.sc$ManualAnno_L2[sce.sc$subcluster %in% c(1)] <- "TREM2+"
sce.sc$ManualAnno_L2[sce.sc$subcluster %in% c(2)] <- "FOLR2+"
sce.sc$ManualAnno_L2[sce.sc$subcluster %in% c(3)] <- "CD48+"
sce.sc$ManualAnno_L2[sce.sc$subcluster %in% c(5)] <- "S100A12+"
sce.sc$ManualAnno_L2[sce.sc$subcluster %in% c(4)] <- "Dying"
...
```{r TcellUMAP}
#Plot the UMAp
plotReducedDim(sce.sc, "SC_UMAP_Tcell" ,colour_by = "subcluster",text_by="subcluster") +
          labs(title="Recalculated UMAP for Subcluster ",subtitle="Colored by clustering")
```
...
sce$ManualAnno_L2[sce$subcluster %in% c(4)] <- "NKTcell"
sce$ManualAnno_L2[sce$subcluster %in% c(3)] <- "CytotoxCD4Tcell"
sce$ManualAnno_L2[sce$subcluster %in% c(1)] <- "CytotoxCD8Tcell"
sce$ManualAnno_L2[sce$subcluster %in% c(1)] <- "NaiveTcell"
sce$ManualAnno_L2[sce$subcluster %in% c(1)] <- "MemoryTcell"
...
#### Test Known Endothelial marker {.tabset}
...
sce.sc$ManualAnno_L2[sce.sc$subcluster %in% c(1)] <- "Arteriolar"
sce.sc$ManualAnno_L2[sce.sc$subcluster %in% c(2)] <- "Venular"
sce.sc$ManualAnno_L2[sce.sc$subcluster %in% c(3)] <- "Capillary"
#sce$ManualAnno_L2[sce$subcluster %in% c(1)] <- "Lymphatic"
```
