...
colData(sce)$louvain <- clusterCells(sce, use.dimred = "MNN", BLUSPARAM = SNNGraphParam(k=15,cluster.fun="louvain"))
...
colData(sce)$leiden <- clusterCells(sce, use.dimred = "MNN", BLUSPARAM = SNNGraphParam(k=40,cluster.fun="leiden"))
...
## Clustering Plots
```{r}
methods <- c("lovain","leiden","walktrap")
```
...
ggplot(data=ClusterInfo, aes(x=Sample,y=cells, fill=i)) +
  geom_bar(stat="identity", position="stack") +
  labs(title=paste0("Absolute comparison nr Cells in ",i," Cluster"),subtitle="Per Sample",x="",y="Number of cells")
  }
```
...
ggplot(data=ClusterInfo,aes(x=Sample, y=cells, fill=i)) +
  ...
  labs(title=paste0("Relative comparison nr Cells in ",i," Cluster"),subtitle="Per Sample",x="",y="Number of cells")
  }
```
