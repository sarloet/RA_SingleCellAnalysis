...
colData(sce.cluster)$louvain <- clusterCells(sce.cluster, use.dimred = "MNN_reduced", BLUSPARAM = SNNGraphParam(k=25,cluster.fun="louvain",type="jaccard",BPPARAM =bpp))
...
colData(sce.cluster)$leiden <- clusterCells(sce.cluster, use.dimred = "MNN_reduced", BLUSPARAM = SNNGraphParam(k=60,cluster.fun="leiden",BPPARAM =bpp))
...
set.seed(123)
colData(sce.cluster)$walktrap <- clusterRows(reducedDim(sce.cluster, "MNN_reduced"),TwoStepParam(KmeansParam(centers=10000, nstart = 5, iter.max = 100), NNGraphParam(k=15,cluster.fun="walktrap",BPPARAM=bpp)))
...
for (i in methods){
  cat(paste0("#### ",i, "\n"))
  print(
  plotReducedDim(sce.cluster, dimred="MNN_reduced", colour_by=i ,text_by=i) + ggtitle(paste0("PCA with ",i," clusters"))
  )
  cat(' \n \n')
  }
```
...
for (i in methods){
  cat(paste0("#### ", i, "\n"))
  print(
  plotReducedDim(sce.cluster, dimred="MNN_UMAP_reduced", colour_by=i,text_by=i) + ggtitle(paste0("UMAP with ",i," clusters"))
  )
  cat(' \n \n')
  }
```
...
```{r UMAPClustering}
plotReducedDim(sce, dimred="MNN_UMAP_reduced", colour_by="cluster",text_by="cluster") + ggtitle("UMAP with assigned clusters")
```

### PCA

```{r PCAClustering}
plotReducedDim(sce, dimred="MNN_reduced", colour_by="cluster",text_by="cluster") + ggtitle("PCA with assigned clusters")
```
