...
colData(sce.cluster)$louvain <- clusterCells(sce.cluster, use.dimred = "MNN", BLUSPARAM = SNNGraphParam(k=10,cluster.fun="louvain",BPPARAM =bpp))
...
colData(sce.cluster)$leiden <- clusterCells(sce.cluster, use.dimred = "MNN", BLUSPARAM = SNNGraphParam(k=70,cluster.fun="leiden",BPPARAM =bpp))
...
set.seed(123)
colData(sce.cluster)$walktrap <- clusterRows(reducedDim(sce.cluster, "MNN"),TwoStepParam(KmeansParam(centers=10000, nstart = 5, iter.max = 100), NNGraphParam(k=15,cluster.fun="walktrap", BPPARAM=bpp)))
