...
colData(sce)$qc.count_manual <- sce$sum < 400 | sce$sum > 100000 # Cells have at least a count of 1000
colData(sce)$qc.genexp_manual <- sce$detected < 300  # Cells have at least 300 genes expressed
...
colData(sce)$qc.count <- isOutlier(sce$sum,nmads=4, log=TRUE, type="both",batch =sce$Sample )
data.frame(attr(colData(sce)$qc.count, "thresholds")) 

# Detect cells with very low number of genes
colData(sce)$qc.genexp <- isOutlier(sce$detected,nmads=4, log=TRUE, type="lower",batch =sce$Sample)
...
```{r QcPlots3}
#Plot Total count
plotColData(sce, x="Sample", y="sum",other_fields="Sample",colour_by = data.frame(factor(sce$qc.count | sce$qc.count_manual,levels=c(FALSE,TRUE)))) + 
...
```{r QcPlots4}
#Plot Detected Genes
plotColData(sce, x="Sample", y="detected", other_fields="Sample",colour_by = data.frame(factor(sce$qc.genexp | sce$qc.genexp_manual,levels=c(FALSE,TRUE))))+ 
    facet_wrap(~Sample, nrow=1, scales = "free_x") + 
    scale_y_log10() + 
