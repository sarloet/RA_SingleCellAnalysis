...
ggplot(colData(sce), aes(x=Sample)) +
  ...
  theme_bw() + stat_count(geom = "text", colour = "white", size = 3.5,aes(label = ..count..),position=position_stack(vjust=0.65))
...
ggplot(colData(sce), aes(x=Joint.Location))+
  ...
  theme_bw() +stat_count(geom = "text", colour = "white", size = 3.5,aes(label = ..count..),position=position_stack(vjust=0.8))
...
#Remove Cells with very low expression of less than 200 genes expressed and cells with low count
...
colData(sce)$qc.count_manual <- sce$sum < 500 | sce$sum > 75000# Cells have at least a count of 1000
colData(sce)$qc.genexp_manual <- sce$detected < 200 # Cells have at least 200 genes expressed
...
data.frame(attr(colData(sce)$qc.count, "thresholds")) 
...
data.frame(attr(colData(sce)$qc.genexp, "thresholds"))
...
data.frame(attr(colData(sce)$qc.mito, "thresholds")) 
...
```{r QcPlots3}
#Plot Total count
plotColData(sce, x="Sample", y="sum",other_fields="Sample",colour_by = data.frame(sce$qc.count | sce$qc.count_manual)) + 
...
```{r QcPlots4}
#Plot Detected Genes
plotColData(sce, x="Sample", y="detected", other_fields="Sample",colour_by = data.frame(sce$qc.genexp | sce$qc.genexp_manual)) + 
    facet_wrap(~Sample, nrow=1, scales = "free_x") + 
    scale_y_log10() + 
...
```{r QcPlots5}
#Plot Mito percent
plotColData(sce, x="Sample", y="subsets_Mito_percent", other_fields="Sample",colour_by = data.frame(sce$qc.mito | sce$qc.mito_manual)) + 
    facet_wrap(~Sample, nrow=1, scales = "free_x") +
...
plot(unname(sizeFactors(sce.lib)), sizeFactors(sce.deconv), xlab="Library size factor",
    ylab="Deconvolution size factor", log='xy', pch=16,
    col=as.integer(factor(clust.filtered)))
