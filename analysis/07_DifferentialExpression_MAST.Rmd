...
find_de_MAST_RE <- function(sce,C_type,cond1,cond2){
  ...
  # create a MAST object
    ...
    result <- merge(summaryDt[contrast== paste0('group',cond2) & component=='H',.(primerid, `Pr(>Chisq)`)], # p-values
                     summaryDt[contrast== paste0('group',cond2) & component=='logFC', .(primerid, coef)],
                     by='primerid') # logFC coefficients
    r<-result
    # MAST uses natural logarithm so we convert the coefficients to log2 base to be comparable to edgeR
    result[,coef_baselog2:=result[,coef]/log(2)]
...
```{r SaveData}
res <- find_de_MAST_RE(sce,C_type="Fibroblast",cond1="Knee",cond2="MCP")
```
...
#res <- result
res <- result[,c("Pr(>Chisq)","coef_baselog2")]
row.names(res)<- result$primerid
EnhancedVolcano(res, lab = row.names(res), x = 'coef_baselog2', y = 'Pr(>Chisq)',
...
```{r SaveData}
write.csv(result, paste0(path,'/output/MAST_KneeVsMCP.csv'), row.names=FALSE)
#write.csv(res, paste0(path,'/output/MAST_KneeVsMCP.csv'), row.names=FALSE)
#write.csv(res, paste0(path,'/output/MAST_KneeVsMCP.csv'), row.names=FALSE)
```
