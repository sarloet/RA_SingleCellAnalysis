...
for (comp in c("KneeVsMCP","KneeVsWrist","MCPVsWrist")){
  ...
  table<-geneList_KneeVsMCP[geneList_KneeVsMCP$Pr..Chisq.<0.05 & geneList_KneeVsMCP$log2FoldChange< -0.1,, drop=F]
  print(table)
  dim(table)
...
for (comp in c("KneeVsMCP","KneeVsWrist","MCPVsWrist")){
  ...
  res_plot <- res_plot[,c("Pr(>Chisq)","log2FoldChange")]
    
  strspl<-strsplit(comp, split = "Vs")#
  
  #Create dd for plotting function
  volcano_plot<-EnhancedVolcano(res_plot, lab = row.names(res_plot), x = 'log2FoldChange', y = 'Pr(>Chisq)',
...
for (comp in c("KneeVsMCP","KneeVsWrist","MCPVsWrist")){
  ...
  res_sig <- res[res$`Pr(>Chisq)`<0.01 & abs(res$log2FoldChange)> 0.1,, drop=F] #genes for cond1
  data.table::setorder(res_sig, Pr..Chisq.)
  res_sig<-head(res_sig,n=25)
  
  # Subset sce to keep the tested conditions
  strspl<-strsplit(comp, split = "Vs")#
  sce.subset<-sce.subset[,sce.subset$Joint.Location == c(strspl[[1]][1], strspl[[1]][2])]
  
  # Aggregate cells by sample and subset for significant genes
  sce.subset.aggr <- aggregateAcrossCells(sce.subset,  ids = sce.subset$Sample, statistics = "mean",use.assay.type = "logcounts", subset.row = res_sig$primerid)
