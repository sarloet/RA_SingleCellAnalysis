...
for (i in seq_along(per.block)) {
    ...
    plot(decX$mean, decX$total, xlab="Mean log-expression", 
        ylab="Variance", main=names(per.block)[i])#col=rowData(sce)$is_hvg
...
intrinsicDimension::maxLikGlobalDimEst(as.matrix(reducedDim(sce, "PCA")), k=10)

```

```{r PCvsVariance}
# Elbow Plot (PC vs variance)
ggplot(pca_pct_variance,aes(PC, variance)) +geom_line(color="grey") +geom_point() +labs(y = "Variance explained (%)")+ geom_vline(xintercept = 12,color="red")

```

```{r SelectPC}
#PCA select nr of PC
reducedDim(sce,"PCA_reduced") <- reducedDim(sce,"PCA")[,seq_len(12)]
reducedDimNames(sce)
