...
```{r PlotFacetPCA1}
#Plot PCA per sample
plotReducedDim(sce, dimred="PCA_reduced", colour_by="Sample",point_size=0.8) + facet_wrap(~sce$Sample)
```

##### PCA per Joint

```{r PlotFacetPCA2}
plotReducedDim(sce, dimred="PCA_reduced", colour_by="Sample",point_size=0.8) + facet_wrap(~sce$Joint.Location)
```
...
```{r PlotFacetUMAP2}
#Plot UMAP before data integration per sample
plotReducedDim(sce, dimred="UMAP", colour_by="Sample") + facet_wrap(~sce$Joint.Location)
```
