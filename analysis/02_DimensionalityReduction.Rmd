...
for (i in seq_along(per.block)) {
    decX <- per.block[[i]]
    plot(decX$mean, decX$total, col= c("orange","black")[(names(metadata(decX)$mean) %in% top.hvgs)+1], xlab="Mean log-expression", 
        ylab="Variance", main=names(per.block)[i] )
    curve(metadata(decX)$trend(x), col="blue", add=TRUE)
    legend("topright", legend=c("Not HVG", "HVG"), col = c("orange","black"),bty="n",pch=10)
}
```
...
ggplot(pca_pct_variance,aes(PC, variance)) +geom_line(color="grey") +geom_point() +labs(y = "Variance explained (%)")+ geom_vline(xintercept = elbow,color="red")+theme_classic()

```

```{r SelectPC}
#PCA select nr of PC
reducedDim(sce,"PCA_reduced") <- reducedDim(sce,"PCA")[,seq_len(20)]
reducedDimNames(sce)
