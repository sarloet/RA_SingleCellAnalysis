...
```{r UMAPAnnotation}
#cell type
plotReducedDim(sce, dimred="MNN_merge_reduced_UMAP", colour_by="Annotation_L0",point_alpha=0.7,point_size=0.1)+
  labs( x='UMAP 1', y='UMAP 2' )+
  guides(colour = guide_legend(override.aes = list(size=2)))
```
...
sce.milo <- makeNhoods(sce.milo, prop = 0.05, k = 40, d = dim, refined = TRUE, reduced_dims = "MNN_merge_reduced",refinement_scheme="graph") #40
...
for (comp in c("KneeVsMCP","KneeVsWrist","MCPVsWrist")){
  ...
  print(ggplot(res, aes(Annotation_L2_fraction)) + geom_histogram(bins=50)+theme_classic())
  ...
  print(plotDAbeeswarm(res, group.by = "Annotation_L2",alpha=0.1)+labs(x="Annotation"))
...
saveRDS(sce.milo, file =paste0(path,'/output/08_DifferentialAbundance_miloR_Myeloid.rds'))
```




## Differential Abundance with miloR Stromal
...
```{r SaveData}
saveRDS(sce.milo, file =paste0(path,'/output/08_DifferentialAbundance_miloR_Stromal.rds'))
```

## Differential Abundance with miloR Endothelial
...
```{r }
#Create milo object
sce.milo<-sce[,sce$Annotation_L0 == "Endothelial"]
dim(sce.milo)
...
## Differential Abundance with miloR Lymphocyte
...
```{r }
#Create milo object
sce.milo<-sce[,sce$Annotation_L0 == "Lymphocyte"]
dim(sce.milo)
