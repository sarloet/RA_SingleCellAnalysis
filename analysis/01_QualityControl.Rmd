...
colData(sce)$qc.hbb_manual <- sce$subsets_Hbb_percent > 5 #Remove cells with ore than 5% HBB content (erythrocytes)
...
data.frame(t(attr(colData(sce)$qc.count, "thresholds"))) #show thresholds
...
data.frame(t(attr(colData(sce)$qc.genexp, "thresholds"))) #show thresholds
...
data.frame(t(attr(colData(sce)$qc.mito, "thresholds"))) #show thresholds
...
#Color by individual cluster
 ...
 data.frame(LibrarySizeFactors = unname(sizeFactors(sce.lib)), DeconvolutionSizeFactors = sizeFactors(sce.deconv),Cluster = as.integer(clust.filtered)) %>%
     ...
     geom_point(aes(col=factor(Cluster)), size=0.5) +
