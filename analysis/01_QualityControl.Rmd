...
colData(sce)$qc.count_manual <- sce$sum < 0 # Cells have at least a count of 400 |sce$sum > 100000
colData(sce)$qc.genexp_manual <- sce$detected < 200 | sce$detected > 10000  # Cells have at least 300 genes expressed 
...
colData(sce)$qc.count <- isOutlier(sce$sum,nmads=2, log=TRUE, type="both",batch =sce$Sample )
data.frame(attr(colData(sce)$qc.count, "thresholds")) 

# Detect cells with very low number of genes
colData(sce)$qc.genexp <- isOutlier(sce$detected,nmads=2, log=TRUE, type="both",batch =sce$Sample)
...
#### Save filtered Data
```{r SaveDataFiltered}
saveRDS(sce.filtered, file =paste0(path,'/output/01_sce_QualityControl_filtered.rds'))
```
